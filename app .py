...
def procesar_callback_oauth():
    ...
    if isinstance(error_oauth, list):
...
def html_mora_bravo(nombre, mora, id_plantilla="T001"):
    ...
    dias_visibles = {"T001": 1, "T030": 30, "T060": 60, "T090": 90}.get(idp)
    if dias_visibles is not None:
        mora_txt = f"{dias_visibles} día en mora" if dias_visibles == 1 else f"{dias_visibles} días en mora"
    else:
...
def _extraer_dias_mora_texto(valor):
...
@st.cache_data(ttl=600, show_spinner=False)
def cargar_dias_mora_comision():
    """Máximo día de mora por referencia usando Comisión, columna M."""
    archivo = obtener_gc().open_by_key(MORA_ESTADOS_SPREADSHEET_ID)
    respuesta = archivo.values_get(f"'{HOJA_MORA_COMISION}'!A:M")
    valores = respuesta.get("values", [])
    if len(valores) <= 1:
        return {}

    encabezados = [normalizar(x).replace(" ", "_") for x in valores[0]]
    i_ref = 0
    for candidato in ("REFERENCIA", "REFERENCE", "REF"):
        if candidato in encabezados:
            i_ref = encabezados.index(candidato)
            break
    i_dias = 12  # columna M, indicada por operación

    salida = {}
    for fila in valores[1:]:
        ref = normalizar_referencia(fila[i_ref] if len(fila) > i_ref else "")
        if not ref:
            continue
        dias = int(numero(fila[i_dias] if len(fila) > i_dias else 0))
        salida[ref] = max(salida.get(ref, 0), dias)
    return salida
...
def excede_limite_mora(referencia, mora_fallback=""):
    return dias_mora_referencia(referencia, mora_fallback) >= 366


# Compatibilidad: ya no significa "Mora 180"; solo queda para llamadas antiguas.
def es_mora_180(valor):
    return _extraer_dias_mora_texto(valor) >= 366
...
def actualizar_estado_pab_proximos(
    ...
    # Los eventos ALDIA vienen de Unidos_Est y son virtuales; la cola ya evita duplicados.
    if str(id_evento or "").startswith("ALDIA-"):
        return
...
def agregar_recordatorio_pab_a_cola(
    ...
    if excede_limite_mora(referencia, mora):
        raise ValueError(
            "Cliente excluido automáticamente por tener 366 días o más en mora."
        )
...
def enviar_pab_hoy_ahora(credenciales):
    ...
    Respeta sin email, límite de mora (<366 días), Excluir_correo, AVISO_HOY y duplicados.
    """
    if credenciales is None:
...
def nombre_plantilla_visible(id_plantilla):
    idp = str(id_plantilla or "").strip().upper()
    nombres_base = {
    ...
    if fila is not None:
        for col in ("NOMBRE_PLANTILLA", "NOMBRE", "DESCRIPCION", "DESCRIPCIÓN"):
            try:
            ...
            if valor:
                return valor
    return nombres_base.get(idp, idp)


def opciones_plantillas_visibles(ids):
    return {f"{nombre_plantilla_visible(i)}  ({i})": i for i in ids}
...
def preparar_clientes_campana(fila_campana):
    ...
    if tipo == "PRUEBA":
        ...
        if tipo == "ALIANZAS":
            ...
            mask_180 = base.apply(
                lambda r: excede_limite_mora(r.get("REFERENCIA", ""), r.get("MORA", "")),
                axis=1,
            )
...
def cargar_pagos_al_dia_unidos():
    """Lee Unidos_Est sin indexar objetos gspread. A=referencia, C=status actual, H=fecha, I=valor."""
    archivo = obtener_gc().open_by_key(UNIDOS_EST_SPREADSHEET_ID)
    respuesta = archivo.values_get(f"'{HOJA_UNIDOS_EST}'!A:I")
    valores = respuesta.get("values", [])
    if len(valores) <= 1:
        return pd.DataFrame()
    ...
    for fila in valores[1:]:
        ...
        status_actual = str(fila[2] if len(fila) > 2 else "").strip()
        fecha = str(fila[7] if len(fila) > 7 else "").strip()
        valor = fila[8] if len(fila) > 8 else ""
        if not ref or normalizar(status_actual) != "AL DIA" or not fecha:
            continue
        fecha_dt = pd.to_datetime(fecha, errors="coerce", dayfirst=True)
        if pd.isna(fecha_dt):
            continue
        filas.append({
            "ID_EVENTO": f"ALDIA-{ref}-{fecha_dt.strftime('%Y%m%d')}",
            "REFERENCIA": ref,
            "FECHA_PAB": fecha_dt.strftime("%d/%m/%Y"),
            "VALOR_PAB": valor,
            "STATUS": "Al día",
    ...
    return pd.DataFrame(filas).drop_duplicates(subset=["REFERENCIA", "FECHA_PAB"], keep="last") if filas else pd.DataFrame()


def enriquecer_pagos_al_dia_con_clientes(df, df_clientes):
    if df.empty:
        return df
    salida = df.copy()
    ...
    mapa = {clave(c): c for c in base.columns}
    def origen(*nombres):
        for n in nombres:
            if clave(n) in mapa:
                return mapa[clave(n)]
        return None
    c_ref = origen("REFERENCIA", "REFERENCE", "REF")
    if not c_ref:
        return salida
    base["_REF"] = base[c_ref].apply(normalizar_referencia)
    base = base.drop_duplicates("_REF", keep="last").set_index("_REF")
    for destino, candidatos in {
        "NOMBRE": ("NOMBRE", "NOMBRE_CLIENTE"),
        "EMAIL": ("EMAIL", "CORREO"),
        "MORA": ("MORA", "MORA_STATUS"),
        "ENCARGADO": ("ENCARGADO", "NEGOCIADOR", "PERSONA"),
    }.items():
        c = origen(*candidatos)
        salida[destino] = salida["REFERENCIA"].map(base[c].to_dict()) if c else ""
    return salida
            ...
            with st.expander(
                ...
                if vista_previa is None:
                    ...
                    if not email_cliente:
                        ...
                        puede_agregar = False
                        motivo_bloqueo = (
                            "Cliente excluido por tener 366 días o más en mora."
                        )
                ...
                st.caption(
                    "Masivos Bravo valida duplicados, mora menor a 366 días y "
    ...
    with tab_nueva:
        ...
        with st.form("form_nueva_campana", clear_on_submit=False):
            ...
            if es_personalizada or es_prueba:
                if ids_plantillas:
                    _opciones_tpl = opciones_plantillas_visibles(ids_plantillas)
                    _etiqueta_tpl = st.selectbox(
                        "Plantilla",
                        list(_opciones_tpl.keys()),
                        help=(
                    ...
                    plantilla_campana = _opciones_tpl[_etiqueta_tpl]
                else:
                    plantilla_campana = ""
                ...
                st.text_input(
                    "Plantilla",
                    value=nombre_plantilla_visible(plantilla_campana),
                    disabled=True,
                ...
                if fila_sel is not None:
                    ...
                    try:
                        ...
                        if tipo_sel == "PRUEBA":
                            ...
                            r4.metric("366+ días mora", resumen.get("mora_180", 0))
                            ...
                            r3.metric("366+ días mora", resumen.get("mora_180", 0))
...
# ============================================================
    ...
    if plantillas.empty or "ID_PLANTILLA" not in plantillas.columns:
        ...
        _opciones_preview = opciones_plantillas_visibles(ids_plantilla)
        _etiqueta_preview = st.selectbox("Plantilla", list(_opciones_preview.keys()), key="plantilla_preview_id")
        id_preview = _opciones_preview[_etiqueta_preview]
